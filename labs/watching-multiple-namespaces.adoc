...
Exit the `root` account and the `master` machine and return to the workstation console.

Assign the two users to the previously created projects:

----
oadm policy add-role-to-user admin dev-team-1 -n team-1
oadm policy add-role-to-user admin dev-team-2 -n team-2
----
...
Now try to repeat the operation.
